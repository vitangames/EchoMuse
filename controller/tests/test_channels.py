...
# This fork publishes a customised GA add-on while deliberately retaining the
# official EA directory byte-for-byte. Regenerating EA from the forked GA
# would point its independent EA version at a non-existent vitangames image
# tag. Upstream channel-parity checks remain active in the official tree; in
# this fork a separate custom-deployment test pins the intentional split.
CUSTOM_GA_FORK = GA.get("url") != "https://github.com/wilbowes/EchoMuse"
pytestmark = pytest.mark.skipif(
    CUSTOM_GA_FORK,
    reason="custom GA fork retains the official upstream EA add-on unchanged",
)
