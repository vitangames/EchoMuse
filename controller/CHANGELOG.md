...
- Adds optional `tts_output_media_player` routing for every satellite. Empty
  preserves the built-in Echo Dot speaker; `media_player.*` redirects only
  the ready Assist TTS response through Home Assistant.
- Keeps continuous microphone audio, wake-word detection, VAD, STT and intent
  handling on their existing paths.
- Uses the satellite's existing ESPHome connection instead of granting the
  add-on broad Home Assistant API access. Device-initiated Home Assistant
  actions must be enabled for that EchoMuse ESPHome integration.
- Based on the official EchoMuse Controller 2.20.2 Home Assistant add-on.
